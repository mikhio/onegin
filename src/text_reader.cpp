...
ReturnCode allocateLinesArr(Text *text) {
  ...
  if (!text->lines)
    tmp = (char **) calloc(text->lines_capacity, sizeof(char));
  else
    tmp = (char **) realloc(text->lines, text->lines_capacity);
  ...
  text->lines = tmp;
  text->lines[text->lines_capacity] = NULL;
...
ReturnCode allocateBuffer(Text *text) {
  ...
  if (!text->buffer)
    ...
    tmp = (char *) realloc(text->buffer, text->buffer_capacity);
